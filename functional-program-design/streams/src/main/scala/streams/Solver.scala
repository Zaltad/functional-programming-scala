...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.to(LazyList).map(x => (x._1, x._2 :: history))
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val more = initial.flatMap(x => newNeighborsOnly(neighborsWithHistory(x._1, x._2), explored))
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from((startBlock, Nil) #:: LazyList.empty, Set.empty)
