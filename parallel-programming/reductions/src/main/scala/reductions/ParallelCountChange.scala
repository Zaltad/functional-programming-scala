...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    if (money == 0) {
      ...
      val sum = countChange(money, coins.tail)
      if (money >= coins.head) {
        return sum + countChange(money - coins.head, coins)
      }
      return sum
    }
    0
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    ...
    if (money == 0) {
      ...
      return parCountChange(money, coins.tail, threshold)
    }
    0
  }
  ...
  def totalCoinsThreshold(totalCoins: Int): Threshold =
    (_, coins) => 3 * coins.size <= 2 * totalCoins
